...
- Single dark theme, no light/dark toggle. Each post gets one **AI-generated symbolic cover** (fal.ai/Recraft, pipeline image step) shown in the public featured + card slots and the dashboard review panel — but **no photographic or stock imagery**. (Reverses the MVP-era "no images" rule; see `feature-plans/per-post-image-generation.md` + the 2026-07-06 decision-log entry.)
